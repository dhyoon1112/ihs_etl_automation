...
def new_stg_table(ETL_Directory, Inbound_File_Data, credentials):
    ...
    print('Executing new_stg_table.py, 6-1')
    import pandas as pd
    import numpy as np
    import pyodbc
    ...
    SQL = ("IF EXISTS"
           f"(SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'{stg_table}') AND type in (N'U'))"
           f"DROP TABLE {stg_table};"
           f"CREATE TABLE {stg_table} (" + SQL[:-2] + ")"
           )
